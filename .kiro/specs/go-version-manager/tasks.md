...
- [ ] 实现版本列表过滤（仅 Linux 平台）
- [ ] 实现版本列表排序（降序）
- [ ] 支持拉取完整历史列表（`include=all`），避免仅显示最新两个版本
- [ ] 通过属性 1（版本列表排序不变性）测试
- [ ] 处理网络错误情况
...
- [ ] 通过属性 6（安装失败清理属性）测试
- [ ] 处理各种错误情况
- [ ] 安装成功后输出 go version/GOROOT/GOPATH 摘要以及 `source` 命令提示
...
**描述**: 探测当前网络的国家代码，命中中国大陆时切换到 studygolang 镜像源，确保远程版本查询与下载在国内环境可用。

**验收标准**:
- [ ] 实现 RegionDetector，调用公网 IP API 获取国家代码并对结果做缓存/超时处理
- [ ] 实现 MirrorSelector，根据国家代码返回远程 API (`https://golang.google.cn/dl/?mode=json`) 与下载基础路径 (`https://studygolang.com/dl/golang/`)
- [ ] 将 remote.Client 改造为可注入镜像配置，并在 CLI 初始化阶段执行探测与切换
- [ ] 探测失败或非中国地区时回退到默认 go.dev 源
- [ ] 在 `remote` 包或新的包中添加单元测试，覆盖 `CN` 与非 `CN` 的镜像选择逻辑（属性 11）
